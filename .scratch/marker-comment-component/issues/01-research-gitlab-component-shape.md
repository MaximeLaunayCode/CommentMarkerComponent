...
Recommended initial inputs: `job-name`, `stage`, mandatory `marker-text`, `patch-path`, `fail-when-patch-needed`, `file-globs`, `exclude-globs`, and `api-token-variable`. Exact file glob semantics remain owned by [Decide File Exclusions](05-decide-file-exclusions.md), and whether report-only behavior is exposed remains owned by [Decide Patch Artifact UX](06-decide-patch-artifact-ux.md).
